...
class Environment():
    def __init__(self, Design, group_num, iteration, destination_path):
        self.event_list = []
        self.clock = 0
    ...
    def Simulate(self):
        ...
        output_path = self.destination_path + '/Design_{}/Event_information_group_{}_itera_{}.csv'
        output_file = output_path.format(self.group_num, self.group_num, self.iteration)
...
for row in range(0,len(df_design)):
    ...
    current_directory = os.path.join(current_directory, r'results_100_new')
    final_directory = os.path.join(current_directory, r'Design_{}'.format(group_num))
    if not os.path.exists(final_directory):
        os.mkdir(final_directory)
    #os.mkdir(final_directory)
    destination_path = 'results_100_new'
    
    for iteration in range(0, max_rep):
        Env = Environment(Design, group_num, iteration, destination_path)
